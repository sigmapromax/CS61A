...
def helper(n, i, cur, dir):
    if i == n:
        return cur
    else:
        if i % 8 == 0 or num_eights(i) > 0:
            return helper(n, i+1, cur-dir, -dir)
        else:
            return helper(n, i+1, cur+dir, dir)
...
def missing_digits(n):
    ...
    if n // 10 == 0:
        ...
        rest, last = n // 10 % 10, n % 10
        if last > rest + 1:
            return missing_digits(n // 10 * 10 + last - 1) + 1
        else:
...
def count_coins(change):
    ...
    True
    """
    def coin_partitions(total, big_coin):
        if total == 0:
            return 1
        elif total < 0:
            return 0
            ...
            return coin_partitions(total - big_coin, big_coin) + coin_partitions(total, descending_coin(big_coin))
    return coin_partitions(change, 25)
...
def move_stack(n, start, end):
    ...
    def destination(n1, n2):
        return 6 - n1 - n2

    if n == 1:
        print_move(start, end)
    else:
        move_stack(n-1, start, destination(start, end))
        print_move(start, end)
        move_stack(n-1, destination(start, end), end)
