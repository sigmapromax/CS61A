...
def picky_piggy(score):
    ...
    if score == 0:
        ...
        # solution 1:
            # mod_times = score % 6
            # if mod_times != 0:
            #     mod_times = 6 - mod_times
        mod_times = 5 - ((score - 1) % 6)
        while mod_times > 0:
            n = n // 10
            mod_times = mod_times - 1
        res = n % 10
        return res
    # END PROBLEM 2
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    def turn(strategy, current_score, opponent_score, dice):
        num_rolls = strategy(current_score, opponent_score)
        add_scores = take_turn(num_rolls, opponent_score, dice, goal)
        current_score = current_score + add_scores
        hog_pile_scores = hog_pile(current_score, opponent_score)
        current_score = current_score + hog_pile_scores
        return current_score

    while score0 < goal and score1 < goal: 
        if who == 0:
            score0 = turn(strategy0, score0, score1, dice)
        else:
            score1 = turn(strategy1, score1, score0, dice)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    def say(current_score, opponent_score):
        score = 0
        if who == 0:
            score = current_score
        else:
        ...
        record = running_high
        if (score - last_score) > record:
            record = score - last_score
            print("{} point(s)! That's a record gain for Player {}!".format(record, who))
        return announce_highest(who, score, record)
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 9
    max_num_rolls, max_avg_scores = 0, 0 
    avg_scores_func = make_averaged(roll_dice, trials_count)
    for i in range(1, 11, 1):
        avg_scores = avg_scores_func(i, dice) 
        if avg_scores > max_avg_scores:
            max_num_rolls = i
            max_avg_scores = avg_scores
...
def picky_piggy_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    scores = picky_piggy(opponent_score)
    if scores >= cutoff:
        return 0
    else:
        return num_rolls
