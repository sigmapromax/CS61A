...
def about(topic):
    ...
    def select(paragraph): 
        ...
        without_symbol_p = ''
        for s in paragraph:
            if str.isdigit(s) or str.isalpha(s) or s == ' ':
                without_symbol_p = without_symbol_p + s
        
        p_list = without_symbol_p.split(' ')
...
def accuracy(typed, reference):
    ...
    if len(typed_words) == 0:
        ...
        while i < len(typed_words) and i < len(reference_words):
            # \s = \s | \t | \n
            t = re.sub('\s', '', typed_words[i])
            r = re.sub('\s', '', reference_words[i])
            if t == r:
                right = right + 1
            i = i + 1
        res = round(right / total * 100, 2)
        return res 
    # END PROBLEM 3
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    # typed_word in valid_words -> that word
    # the word with lowest difference from typed_word in valid_words(lowest difference < limit, otherwise return typed_word instead) 
    lowest_diff = -1
    lowest_diff_word = typed_word
    for v in valid_words:
        if typed_word == v:
            return typed_word
        else:
            diff = diff_function(typed_word, v, limit)
            if diff <= limit and (diff < lowest_diff or lowest_diff == -1):
                lowest_diff = diff
                lowest_diff_word = v
...
def time_per_word(words, times_per_player):
    ...
    for i in range(len(times_per_player)):
        ...
        for j in range(len(times_per_player[0]) - 1):
            li.append(times_per_player[i][j + 1] - times_per_player[i][j])
        times.append(li)
...
def fastest_words(match):
    ...
    for i in range(len(player_indices)):
        li = []
        words_matrix.append(li)
    times = get_times(match)
    words = get_words(match)

    # i -> player number, j -> word index
    for j in word_indices:
        shortest_time_index = (0, j)
        shortest_time = times[0][j]
        for i in player_indices:
            if times[i][j] < shortest_time:
                shortest_time_index = (i, j)
                shortest_time = times[i][j]
        player_num, word_index = shortest_time_index[0], shortest_time_index[1]
        words_matrix[player_num].append(words[word_index])
    return words_matrix
    # END PROBLEM 10
